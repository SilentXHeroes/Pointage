(jQuery)(function($){
    ...
    $.each(stats, function(year, months){
        html += '<div class="year"><hr><span>'+year+'</span></div>';
        $.each(months, function(month, days){
            html += '<div class="month" data-month="'+month+'">';
            $.each(days, function(day, ptg){
                html += '<div class="day">'
                            +'<div class="date"><span class="icon-calendar"></span> '+week[ptg.id_day]+' '+ day +' '+ listMonths[month] +'</div>'
                            +'<hr>'
                            +'<div class="timeWorked">'
                                +'<p>Vous avez travaillé</p>'
                                +'<p>'+ptg.time.H+' heures et '+ptg.time.M+' minutes</p>'
                            +'</div>'
                            +'<div class="pointages">'
                                + '<p>' + ptg.ptg.join('</p><span class="icon-arrow-down" style="transform: rotate(-90deg);"></span> <p>') + '</p>'
                            +'</div>'
                        +'</div>';
            });
            html += '</div>';
